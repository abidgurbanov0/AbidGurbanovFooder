...
class UserPageVC: UIViewController {
    ...
    @IBAction func buttonDuzenle(_ sender: Any) {
        ...
        if isEditinInfo{
            let alert = UIAlertController(title: "Duzelis", message: "Duzeltmek isteyirsiz ? ", preferredStyle: .alert)
    ...
    @IBAction func buttonLogout(_ sender: Any) {
        ...
        let alert = UIAlertController(title: "Çıxis", message: "Cixis etmekisteyirsiz ? ", preferredStyle: .alert)
...
extension UserPageVC:UICollectionViewDelegate,UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let url = URL(string: "\(url)\(favFood.image!)"){
            DispatchQueue.main.async {
                cell.imageFood.kf.setImage(with : url)
            }
            
        }
        cell.labelFoodName.text = favFood.name
        cell.delegate = self
    ...
    func cellDesign(){
        let designe = UICollectionViewFlowLayout()
        
         designe.sectionInset = UIEdgeInsets(top: 10, left: 10, bottom: 10, right: 10)
         designe.minimumInteritemSpacing = 0
         designe.minimumLineSpacing = 15
         
         
        let lasrgeOf = favFoodsCollectionView.bounds.width-20
         designe.itemSize = CGSize(width: lasrgeOf, height: 70)
        
        favFoodsCollectionView.collectionViewLayout = designe
    }
}
extension UserPageVC : FavFoodUserPage{
    func favFood(indexPath: IndexPath) {
        userPagePresenterObject?.deleteFoodFromFavList(yemek_adi: favFoodsList[indexPath.row].name!)
